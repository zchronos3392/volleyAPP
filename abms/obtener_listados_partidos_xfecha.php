...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
	...
	//CHEQUEO SI VIENEN LOS FILTROS PARA AGGREAR
		...
		// agregado como funcion para visualizar errores en abmpartidos.sept2024
		$codigoError = 0;
		if (isset($_GET["errorCodigo"]))	$codigoError = (int) $_GET["errorCodigo"];			
		// agregado como funcion para visualizar errores en abmpartidos.sept2024
		...
		$codigopartido = '';
		if(isset($_GET["codigoP"])) $codigopartido = $_GET["codigoP"];		
	    ...
	    if($registros["0"]["count(*)"] > "0")
	     {
			...
			{
				//$codigopartido agregado tambien en obtener_partidos.php
            	$partidos = Partido::getAllcparms($icomp,$icate,$iclub,$ClubNombre,$icity,$fecDde,$fecHta,$fecOrden,$estado,99,$codigoError,$codigopartido);
            }
			// else				
		    ...
		    {
				$datos["estado"] = 0;
		        $datos["Partidos"] = Array();//es un array
					print json_encode($datos);
		    }
		}
		else
